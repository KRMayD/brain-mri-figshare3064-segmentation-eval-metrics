...
def postprocess_kmeans(args):
    ...
    for file in tqdm(files):
        ...
        # 원본 saliency map (0~1 사이)
        attn_weights = cv2.imread(os.path.join(args.sal_path, file), 0) / 255.0
        h, w = attn_weights.shape

        # kmeans는 256x256에서 수행
        ...
        # ---- 여기부터 필터링 로직 수정 ----
        if args.filter:
            nb_blobs, labels_cc, stats, _ = cv2.connectedComponentsWithStats(segmented_image)
            
            # 성분이 아무 것도 없으면 그대로 저장
            if nb_blobs <= 1:
                cv2.imwrite(os.path.join(args.output_path, file), segmented_image)
                continue

            # num_contours == 1 이면 "가장 밝은 지점"이 속한 성분만 남김
            if args.num_contours == 1:
                # 원본 saliency map에서 가장 밝은 픽셀 위치
                max_pos = np.unravel_index(np.argmax(attn_weights), attn_weights.shape)
                target_label = labels_cc[max_pos]

                # 만약 max_pos가 배경(0)이면, 기존 방식(면적 최대)으로 fallback
                if target_label == 0:
                    sizes = stats[1:, cv2.CC_STAT_AREA]
                    largest_idx = 1 + np.argmax(sizes)  # stats에서 0은 배경
                    im_result = np.zeros_like(labels_cc, dtype=np.uint8)
                    im_result[labels_cc == largest_idx] = 255
                else:
                    im_result = np.zeros_like(labels_cc, dtype=np.uint8)
                    im_result[labels_cc == target_label] = 255

                segmented_image = im_result

            else:
                # num_contours > 1 인 경우 기존 "넓이 상위 K개" 방식 유지
                sizes = stats[:, cv2.CC_STAT_AREA]
                sorted_sizes = sorted(sizes[1:], reverse=True) 
                top_k_sizes = sorted_sizes[:args.num_contours]
                ...
                for index_blob in range(1, nb_blobs):
                    if sizes[index_blob] in top_k_sizes:
                        im_result[labels_cc == index_blob] = 255
                segmented_image = im_result
        # ---- 필터링 끝 ----
