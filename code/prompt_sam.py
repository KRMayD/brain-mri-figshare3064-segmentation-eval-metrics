...
def write_mask_to_folder(mask , t_mask, path: str,num_contours) -> None:
    ...
    if not cv2.imwrite(output_path, mask):
        raise RuntimeError(f"Could not write SAM mask: {output_path}")
...
def scoremap2bbox(scoremap, multi_contour_eval=False):
    ...
    if len(contours) == 0:
        return np.asarray([[0, 0, width, height]]), [], 0
...
def main(args: argparse.Namespace) -> None:
    ...
    def collect_by_stem(path, label):
        root = Path(path)
        ...
        for file_path in files:
            if file_path.stem in result:
                raise ValueError(
                    f"Duplicate {label} basename '{file_path.stem}': "
                    f"{result[file_path.stem]} and {file_path}"
                )
            result[file_path.stem] = file_path
        if not result:
            raise ValueError(f"No {label} files found at {path}")
        return result

    images_by_stem = collect_by_stem(args.input, "image")
    masks_by_stem = collect_by_stem(args.mask_input, "coarse mask")
    missing_masks = sorted(set(images_by_stem) - set(masks_by_stem))
    extra_masks = sorted(set(masks_by_stem) - set(images_by_stem))
    if missing_masks or extra_masks:
        raise ValueError(
            "Image/coarse-mask basename mismatch: "
            f"missing_masks={missing_masks[:10]} ({len(missing_masks)} total), "
            f"extra_masks={extra_masks[:10]} ({len(extra_masks)} total)"
        )

    pairs = [
        (str(images_by_stem[stem]), str(masks_by_stem[stem]))
        for stem in sorted(images_by_stem)
    ]

    os.makedirs(args.output, exist_ok=True)

    for t, t_mask in tqdm(pairs, total=len(pairs)):

        image = cv2.imread(t)
        if image is None:
            raise RuntimeError(f"Could not load input image: {t}")

        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        mask = cv2.imread(t_mask, cv2.IMREAD_GRAYSCALE)
        if mask is None:
            raise RuntimeError(f"Could not load coarse mask: {t_mask}")
