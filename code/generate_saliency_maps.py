...
def main(args):
    ...
    local_model_dir = getattr(args, "local_model_dir", None) or LOCAL_MODEL_DIR
    if(args.model_name == "BiomedCLIP" and args.finetuned):
        model = AutoModel.from_pretrained(local_model_dir, trust_remote_code=True).to(args.device)
    ...
    for image_id in tqdm(sorted(os.listdir(args.input_path))):
        ...
        if(args.reproduce):
            ...
            print(f"DEBUG: Processing {image_id} with text: {text[:50]}...", flush=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--local-model-dir', type=str, default=None, help='Local HF-format model directory for finetuned BiomedCLIP weights')
